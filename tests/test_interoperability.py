...
class TestInteroperability(unittest.TestCase):
    ...
    def _run_interoperability_test(self, mode, test_data):
        """Универсальный метод для тестирования одного режима."""
        ...
        try:
            result = subprocess.run([
                'cryptocore', '--algorithm', 'aes', '--mode', mode,
            ...
            self.assertEqual(decrypted_data, test_data, f"[{mode}] Data mismatch after round-trip")

        finally:
            self._safe_cleanup(input_file_name, encrypted_file, decrypted_file)

    def test_interoperability_ecb(self):
        """Тест совместимости для режима ECB"""
        self._run_interoperability_test('ecb', b"ECB test data - block aligned exactly 32 bytes!!")

    def test_interoperability_cbc(self):
        """Тест совместимости для режима CBC"""
        self._run_interoperability_test('cbc', b"Test data for interoperability check - AES-CBC")

    def test_interoperability_cfb(self):
        """Тест совместимости для режима CFB"""
    ...
    def test_interoperability_various_lengths(self):
        """Дополнительная проверка: все режимы работают с разной длиной данных"""
        test_cases = [
            b"A",
            b"Short",
            b"Exactly 16 bytes!!",
            b"X" * 17,
            b"Long data string for testing interoperability across different input sizes."
        ]
