...
def validate_key(key_hex):
    try:
        ...
        if len(key_bytes) not in (16, 24, 32):
            raise ValueError("AES key must be 16, 24, or 32 bytes (32, 48, or 64 hex chars)")
        return key_bytes
...
def run_crypto_command(args):
    try:
        ...
        if args.mode == 'gcm':
            aad = bytes.fromhex(args.aad) if args.aad else b""  # ← КЛЮЧЕВОЕ ИЗМЕНЕНИЕ
...
def main():
    parser = argparse.ArgumentParser(
        description='CryptoCore - AES Encryption, Hashing, HMAC, CMAC, and AEAD (GCM)',
        prog='cryptocore'
    )
    ...
    dgst_parser.add_argument('--output', help='Output file to write result (optional)')
    dgst_parser.add_argument('--hmac', action='store_true', help='Use HMAC (requires --key and --algorithm sha256)')
    dgst_parser.add_argument('--cmac', action='store_true', help='Use AES-CMAC (requires --key and --algorithm aes)')
    dgst_parser.add_argument('--key', help='Key for HMAC or CMAC (hex string)')
    dgst_parser.add_argument('--verify', help='Verify against a file containing expected hash/MAC')

    crypto_parser = subparsers.add_parser('crypto', help='AES encryption/decryption (including GCM)')
    crypto_parser.add_argument('--algorithm', required=True, choices=['aes'], help='Cipher algorithm (only aes supported)')
    ...
    crypto_parser.add_argument('--key', help='Encryption key as hex string (16/24/32 bytes for AES)')
    ...
    crypto_parser.add_argument('--iv', help='(Optional) IV as hex string (16 bytes). If omitted, IV is read from input file.')
    crypto_parser.add_argument('--aad', help='Additional Authenticated Data (hex string, for GCM only)')
