...
def main():
    parser = argparse.ArgumentParser(
        description='CryptoCore - AES Encryption, Hashing, HMAC, and CMAC Tool',
        prog='cryptocore'
    )
    ...
    dgst_parser.add_argument('--output',
                             help='Output file to write result (optional)')
    dgst_parser.add_argument('--hmac', action='store_true',
                             help='Use HMAC (requires --key and --algorithm sha256)')
    dgst_parser.add_argument('--cmac', action='store_true',
                             help='Use AES-CMAC (requires --key and --algorithm aes)')
    dgst_parser.add_argument('--key',
                             help='Key for HMAC or CMAC (hex string)')
    dgst_parser.add_argument('--verify',
                             help='Verify against a file containing expected hash/MAC')
